...
---
# <a name="upload-and-create-a-linux-vm-from-custom-disk-with-the-azure-cli-20"></a>Upload and create a Linux VM from custom disk with the Azure CLI 2.0
...
## <a name="prepare-the-disk-to-be-uploaded"></a>Prepare the disk to be uploaded
...
The following example creates a resource group named `myResourceGroup` in the `westus` location: [Azure Managed Disks overview](../../storage/storage-managed-disks-overview.md)
```azurecli
az group create --name myResourceGroup --location westus
```
...
## <a name="create-vm-from-custom-disk"></a>Create VM from custom disk
...
### <a name="azure-20---unmanaged-disks"></a>Azure 2.0 - unmanaged disks
To create a VM with unmanaged disks, specify the URI to your disk (`--image`) with [az vm create](/cli/azure/vm#create). The following example creates a VM named `myVM` using the virtual disk previously uploaded:
...
### <a name="resource-manager-template---unmanaged-disks"></a>Resource Manager template - unmanaged disks
