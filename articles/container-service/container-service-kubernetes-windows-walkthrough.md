...
---
# <a name="get-started-with-kubernetes-and-windows-containers-in-container-service"></a>Get started with Kubernetes and Windows containers in Container Service
...
* The Windows agent nodes are grouped in an Azure availability set and run your containers. The Windows nodes can be accessed through an RDP SSH tunnel via the master node. Azure load balancer rules are dynamically added to the cluster depending on exposed services.
...
## <a name="start-the-application"></a>Start the application
...
## <a name="access-the-windows-nodes"></a>Access the Windows nodes
