...
---
# <a name="copy-data-to-or-from-azure-blob-storage-using-azure-data-factory"></a>Copy data to or from Azure Blob Storage using Azure Data Factory
...
### <a name="recursive-and-copybehavior-examples"></a>recursive and copyBehavior examples
...
## <a name="walkthrough-use-copy-wizard-to-copy-data-tofrom-blob-storage"></a>Walkthrough: Use Copy Wizard to copy data to/from Blob Storage
...
3. In the **New data factory** pane:   
    ...
    6. Click **Create**.
3. After the creation is complete, you see the **Data Factory** blade as shown in the following image: ![Data factory home page](./media/data-factory-azure-blob-connector/data-factory-home-page.png)

### <a name="copy-wizard"></a>Copy Wizard
...
7. On the **File format settings** page, you see the delimiters and the schema that is auto-detected by the wizard by parsing the file. 
    1. Confirm the following options: a. The **file format** is set to **Text format**. You can see all the supported formats in the drop-down list. For example: JSON, Avro, ORC, Parquet.
...
### <a name="monitor-the-pipeline-copy-task"></a>Monitor the pipeline (copy task)

1. Click the link `Click here to monitor copy pipeline` on the **Deployment** page. 
2. You should see the **Monitor and Manage application** in a separate tab.  ![Monitor and Manage App](media/data-factory-azure-blob-connector/monitor-manage-app.png)
...
##### <a name="source-blob-storage-linked-service"></a>Source blob storage linked service
```json
{
...
##### <a name="destination-blob-storage-linked-service"></a>Destination blob storage linked service
...
## <a name="json-examples-for-copying-data-to-and-from-blob-storage"></a>JSON examples for copying data to and from Blob Storage  
...
### <a name="json-example-copy-data-from-blob-storage-to-sql-database"></a>JSON Example: Copy data from Blob Storage to SQL Database
...
```
### <a name="json-example-copy-data-from-azure-sql-to-azure-blob"></a>JSON Example: Copy data from Azure SQL to Azure Blob
The following sample shows:
