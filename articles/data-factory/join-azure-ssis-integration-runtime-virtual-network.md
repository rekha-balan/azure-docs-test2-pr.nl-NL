...
---
# <a name="join-an-azure-ssis-integration-runtime-to-a-virtual-network"></a>Join an Azure-SSIS integration runtime to a virtual network
...
## <a name="access-to-on-premises-data-stores"></a>Access to on-premises data stores
...
## <a name="host-the-ssis-catalog-database-in-azure-sql-database-with-virtual-network-service-endpointsmanaged-instance-preview"></a>Host the SSIS Catalog database in Azure SQL Database with virtual network service endpoints/Managed Instance (Preview)
...
## <a name="requirements-for-virtual-network-configuration"></a>Requirements for virtual network configuration
...
### <a name="use-the-portal-to-configure-an-azure-resource-manager-virtual-network"></a>Use the portal to configure an Azure Resource Manager virtual network
...
### <a name="use-the-portal-to-configure-a-classic-virtual-network"></a>Use the portal to configure a classic virtual network
You need to configure a virtual network before you can join an Azure-SSIS IR to it. 
...
### <a name="join-the-azure-ssis-ir-to-a-virtual-network"></a>Join the Azure-SSIS IR to a virtual network
1. Start Microsoft Edge or Google Chrome. Currently, the Data Factory UI is supported only in those web browsers. 
...
### <a name="create-an-azure-ssis-ir-and-join-it-to-a-virtual-network"></a>Create an Azure-SSIS IR and join it to a virtual network
...
### <a name="join-an-existing-azure-ssis-ir-to-a-virtual-network"></a>Join an existing Azure-SSIS IR to a virtual network
...
### <a name="define-the-variables"></a>Define the variables
```powershell
...
### <a name="stop-the-azure-ssis-ir"></a>Stop the Azure-SSIS IR
...
### <a name="configure-virtual-network-settings-for-the-azure-ssis-ir-to-join"></a>Configure virtual network settings for the Azure-SSIS IR to join
```powershell
# Make sure to run this script against the subscription to which the virtual network belongs.
if(![string]::IsNullOrEmpty($VnetId) -and ![string]::IsNullOrEmpty($SubnetName))
{
...
### <a name="configure-the-azure-ssis-ir"></a>Configure the Azure-SSIS IR
...
### <a name="start-the-azure-ssis-ir"></a>Start the Azure-SSIS IR
