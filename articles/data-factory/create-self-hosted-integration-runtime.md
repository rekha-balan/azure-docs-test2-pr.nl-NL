...
---
# <a name="how-to-create-and-configure-self-hosted-integration-runtime"></a>How to create and configure Self-hosted Integration Runtime
...
## <a name="high-level-steps-to-install-self-hosted-ir"></a>High-level steps to install self-hosted IR
...
## <a name="setting-up-self-hosted-ir-on-azure-vm-using-azure-resource-manager-template-automation"></a>Setting up self-hosted IR on Azure VM using Azure Resource Manager Template (automation)
...
## <a name="considerations-for-using-self-hosted-ir"></a>Considerations for using self-hosted IR
...
## <a name="install-and-register-self-hosted-ir-from-download-center"></a>Install and Register Self-hosted IR from download center
...
## <a name="high-availability-and-scalability"></a>High Availability and Scalability
...
### <a name="scale-considerations"></a>Scale considerations
...
### <a name="tlsssl-certificate-requirements"></a>TLS/SSL certificate requirements
...
## <a name="sharing-the-self-hosted-integration-runtime-ir-with-multiple-data-factories"></a>Sharing the self-hosted Integration Runtime (IR) with multiple data factories
...
#### <a name="high-level-steps-for-creating-a-linked-self-hosted-ir"></a>High level steps for creating a Linked self-hosted IR
...
#### <a name="known-limitations-of-self-hosted-ir-sharing"></a>Known limitations of self-hosted IR sharing
...
2. The data factory in which linked IR is to be created must have an MSI ([managed service identity](https://docs.microsoft.com/azure/active-directory/managed-service-identity/overview)). By default, the data factories created in Ibiza portal or PowerShell cmdlets will have MSI created implicitly. However, in some cases when data factory is created using an Azure Resorce Manager template or SDK, the “**Identity**” **property must be set** explicitly to ensure Azure Resorce Manager creates a data factory containing an MSI. 
...
6. The Azure PowerShell which support this feature is version >= 6.6.0 (AzureRM.DataFactoryV2 >= 0.5.7)
...
### <a name="copy-data-from-a-source-to-a-sink"></a>Copy data from a source to a sink
...
### <a name="configure-proxy-server-settings"></a>Configure proxy server settings
...
### <a name="enable-remote-access-from-intranet"></a>Enable Remote Access from Intranet  
In case if you use **PowerShell** or **Credential Manager application**  to encrypt credentials from another machine (in the network) other than where the self-hosted integration runtime is installed, then you would require the **'Remote Access from Intranet'** option to be enabled. If you run the **PowerShell** or **Credential Manager application**  to encrypt credential on the same machine where the self-hosted integration runtime is installed, then **'Remote Access from Intranet'** may not be enabled.
