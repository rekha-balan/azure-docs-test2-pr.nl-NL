...
---
# <a name="tutorial-load-balance-internal-traffic-with-basic-load-balancer-to-vms-using-the-azure-portal"></a>Tutorial: Load balance internal traffic with Basic Load Balancer to VMs using the Azure portal
...
3. Click **Create** to create the load balancer.
    ...
    ## <a name="create-backend-servers"></a>Create backend servers
...
### <a name="install-iis-and-customize-the-default-web-page"></a>Install IIS and customize the default web page
...
## <a name="create-a-virtual-machine-to-test-the-load-balancer"></a>Create a virtual machine to test the load balancer
...
1. In the Azure portal, get the Private IP address for the Load Balancer on the **Overview** screen. To do so: a. Click **All resources** in the left-hand menu, and then click **myLoadBalancer** from the resources list.
    b. In the **Overview** details page, copy the Private IP address (in this example, it is 10.1.0.7).

2. Create a remote connection to *myVMTest* as follows: a. Click **All resources** in the left-hand menu, and then from the resources list click **myVMTest** that is located in the *myResourceGroupILB* resource group.
