...
---
# <a name="about-keys-secrets-and-certificates"></a>About keys, secrets, and certificates
...
## <a name="key-vault-general-details"></a>Key Vault general details
...
## <a name="key-vault-keys"></a>Key Vault keys
...
-   **Sign and Verify**: Strictly, this operation is "sign hash" or “verify hash” as Azure Key Vault does not support hashing of content as part of signature creation. Applications should hash the data to be signed locally and then request that Azure Key Vault sign the hash. Verification of signed hashes is supported as a convenience operation for applications that may not have access to [public] key material; we recommend that, for best application performance, verify operations are performed locally.  
...
## <a name="key-vault-secrets"></a>Key Vault secrets 
...
## <a name="key-vault-certificates"></a>Key Vault Certificates
...
#### <a name="x509-to-key-vault-usage-mapping"></a>X509 to Key Vault usage mapping
