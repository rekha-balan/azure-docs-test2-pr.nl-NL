...
1. In the search box, type **Domo**.

    ![Creating an Azure AD test user](./media/domo-tutorial/tutorial_domo_search.png)

1. In the results panel, select **Domo**, and then click **Add** button to add the application.
...
1. **[Creating a Domo test user](#creating-a-domo-test-user)** - to have a counterpart of Britta Simon in Domo that is linked to the Azure AD representation of user.
...
1. On the **Domo Domain and URLs** section, perform the following steps:
...
1. Domo application expects the SAML assertions in a specific format. Configure the following claims for this application. You can manage the values of these attributes from the "**User Attributes**" section on application integration page. The following screenshot shows an example for this configuration. 
...
1. On the **Domo Configuration** section, click **Configure Domo** to open **Configure sign-on** window. Copy the **Sign-Out URL, SAML Entity ID, and SAML Single Sign-On Service URL** from the **Quick Reference section.**   

   ![Configure Single Sign-On](./media/domo-tutorial/tutorial_domo_configure.png) 

1. To configure single sign-on on **Domo** side, please navigate to Domo's Knowledge Base article found [here](http://knowledge.domo.com?cid=azuread), and follow the instructions.
