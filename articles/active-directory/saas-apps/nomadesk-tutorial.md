...
## <a name="adding-nomadesk-from-the-gallery"></a>Adding Nomadesk from the gallery
...
1. In the search box, type **Nomadesk**.

    ![Creating an Azure AD test user](./media/nomadesk-tutorial/tutorial_nomadesk_search.png)

1. In the results panel, select **Nomadesk**, and then click **Add** button to add the application.
...
1. **[Creating a Nomadesk test user](#creating-a-nomadesk-test-user)** - to have a counterpart of Britta Simon in Nomadesk that is linked to the Azure AD representation of user.
...
1. On the **Nomadesk Domain and URLs** section, perform the following steps:
...
1. On the **Nomadesk Configuration** section, click **Configure Nomadesk** to open **Configure sign-on** window. Copy the **Sign-Out URL, SAML Entity ID, and SAML Single Sign-On Service URL** from the **Quick Reference section.**

    ![Configure Single Sign-On](./media/nomadesk-tutorial/tutorial_nomadesk_configure.png) 

1. To configure single sign-on on **Nomadesk** side, you need to send the downloaded **Certificate**, **Sign-Out URL, SAML Entity ID, and SAML Single Sign-On Service URL** to [Nomadesk support team](mailto:support@nomadesk.com). They set this setting to have the SAML SSO connection set properly on both sides.
...
### <a name="creating-a-nomadesk-test-user"></a>Creating a Nomadesk test user
