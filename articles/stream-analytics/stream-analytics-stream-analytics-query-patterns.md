...
## <a name="work-with-complex-data-types-in-json-and-avro"></a>Work with complex Data Types in JSON and AVRO 
...
## <a name="query-example-convert-data-types"></a>Query example: Convert data types
...
**Explanation**: Use a **CAST** statement in the **Weight** field to specify its data type. See the list of supported data types in [Data types (Azure Stream Analytics)](https://msdn.microsoft.com/library/azure/dn835065.aspx).

## <a name="query-example-use-likenot-like-to-do-pattern-matching"></a>Query example: Use Like/Not like to do pattern matching
...
**Explanation**: Use the **LIKE** statement to check the **LicensePlate** field value. It should start with an A, then have any string of zero or more characters, and then end with a 9. 
...
**Explanation**: The **CASE** expression compares an expression to a set of simple expressions to determine the result. In this example, vehicle makes with a count of 1 returned a different string description than vehicle makes with a count other than 1. 
...
**Explanation**: The **INTO** clause tells Stream Analytics which of the outputs to write the data to from this statement.
...
## <a name="query-example-count-unique-values"></a>Query example: Count unique values
...
## <a name="query-example-find-the-first-event-in-a-window"></a>Query example: Find the first event in a window
...
## <a name="query-example-find-the-last-event-in-a-window"></a>Query example: Find the last event in a window
...
**Explanation**: There are two steps in the query. The first one finds the latest time stamp in 10-minute windows. The second step joins the results of the first query with the original stream to find the events that match the last time stamps in each window. 
...
**Explanation**: Use **LAG** to peek into the input stream one event back and get the **Make** value. Compare it to the **MAKE** value in the current event, and then output the event if they are the same. You can also use **LAG** to get data about the previous car.

## <a name="query-example-detect-the-duration-between-events"></a>Query example: Detect the duration between events
...
**Explanation**: Use the **LAST** function to retrieve the last **TIME** value when the event type was **Start**. The **LAST** function uses **PARTITION BY [user]** to indicate that the result is computed per unique user. The query has a 1-hour maximum threshold for the time difference between **Start** and **Stop** events, but is configurable as needed **(LIMIT DURATION(hour, 1)**.

## <a name="query-example-detect-the-duration-of-a-condition"></a>Query example: Detect the duration of a condition
...
**Explanation**: Use **LAG** to view the input stream for 24 hours and look for instances where **StartFault** and **StopFault** are spanned by the weight < 20000.
...
**Explanation**: This query generates events every 5 seconds and outputs the last event that was received previously. The [Hopping window](https://msdn.microsoft.com/library/dn835041.aspx "Hopping window--Azure Stream Analytics") duration determines how far back the query looks to find the latest event (300 seconds in this example).


## <a name="query-example-correlate-two-event-types-within-the-same-stream"></a>Query example: Correlate two event types within the same stream
...
**Explanation**: The first query `max_power_during_last_3_mins`, uses the [Sliding window](https://msdn.microsoft.com/azure/stream-analytics/reference/sliding-window-azure-stream-analytics) to find the max value of the power sensor for every device, during the last 3 minutes. The second query is joined to the first query to find the power value in the most recent window relevant for the current event. And then, provided the conditions are met, an alert is generated for the device.

## <a name="query-example-process-events-independent-of-device-clock-skew-substreams"></a>Query example: Process events independent of Device Clock Skew (substreams)
