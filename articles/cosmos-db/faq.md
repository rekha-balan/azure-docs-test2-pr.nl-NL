...
## <a name="azure-cosmos-db-fundamentals"></a>Azure Cosmos DB fundamentals

### <a name="what-happened-to-the-documentdb-api"></a>What happened to the DocumentDB API?
...
### <a name="what-happened-to-azure-documentdb-as-a-service"></a>What happened to Azure DocumentDB as a service?

The Azure DocumentDB service is now a part of the Azure Cosmos DB service and manifests itself in the form of the SQL API. Applications built against Azure DocumentDB will run without any changes against Azure Cosmos DB SQL API. In addition, Azure Cosmos DB supports the Gremlin API, Table API, MongoDB API, and Cassandra API (Preview).
...
Azure Cosmos DB is available in the Azure portal. First, sign up for an Azure subscription. After you've signed up, you can add a SQL API, Gremlin API, Table API, MongoDB API, or Cassandra API account to your Azure subscription.
...
### <a name="how-do-i-create-fixed-collection-with-partition-key"></a>How do I create fixed collection with partition key
...
### <a name="i-have-setup-my-container-to-use-lazy-indexing-i-see-that-my-queries-do-not-return-expected-results"></a>I have setup my container to use lazy indexing, I see that my queries do not return expected results. 
...
### <a name="what-is-the-azure-cosmos-db-api-for-mongodb"></a>What is the Azure Cosmos DB API for MongoDB?
...
### <a name="how-do-i-connect-to-my-api-for-mongodb-database"></a>How do I connect to my API for MongoDB database?
...
### <a name="how-do-i-view-table-data-that-is-stored-in-azure-cosmos-db-for-use-with-the-table-api"></a>How do I view table data that is stored in Azure Cosmos DB for use with the Table API? 
...
### <a name="do-i-need-to-migrate-my-existing-azure-table-storage-applications-to-the-sdk-if-i-do-not-want-to-use-the-table-api-features"></a>Do I need to migrate my existing Azure Table storage applications to the SDK if I do not want to use the Table API features?
...
### <a name="how-do-i-add-replication-of-the-data-in-the-table-api-across-multiple-regions-of-azure"></a>How do I add replication of the data in the Table API across multiple regions of Azure?
...
### <a name="does-this-mean-i-do-not-have-to-create-multiple-indexes-to-satisfy-the-queries"></a>Does this mean I do not have to create multiple indexes to satisfy the queries? 
...
### <a name="azure-table-storage-has-been-very-inexpensive-for-me-because-i-pay-only-to-store-the-data-and-i-rarely-query-the-azure-cosmos-db-table-api-offering-seems-to-be-charging-me-even-though-i-have-not-performed-a-single-transaction-or-stored-anything-can-you-please-explain"></a>Azure Table storage has been very inexpensive for me, because I pay only to store the data, and I rarely query. The Azure Cosmos DB Table API offering seems to be charging me even though I have not performed a single transaction or stored anything. Can you please explain?
...
This error means that the allocated Request Units per second are not enough to serve the query. This error is usually seen when you run a query that obtains all vertices:
...
A Gremlin connection is made through a WebSocket connection. Although WebSocket connections do not have a specific time to live, Azure Cosmos DB Gremlin API will terminate idle connections after 30 minutes of inactivity. 
...
Fluent API calls are not yet supported by the Azure Cosmos DB Gremlin API. Fluent API calls require an internal formatting feature known as bytecode support which is currently not supported by Azure Cosmos DB Gremlin API. Due to the same reason, the latest Gremlin-JavaScript driver is also currently not supported. 
...
The output of the above profile shows how much time is spent obtaining the Vertex and Edge objects, as well as the size of the working data set. This is related to the standard cost measurements for Azure Cosmos DB queries.

## <a id="cassandra"></a> Cassandra API

### <a name="what-is-the-protocol-version-supported-in-the-private-preview-is-there-a-plan-to-support-other-protocols"></a>What is the protocol version supported in the private preview? Is there a plan to support other protocols?
...
### <a name="what-happens-when-throughput-is-exceeded"></a>What happens when throughput is exceeded? 
...
### <a name="is-it-possible-to-create-multiple-tables-with-apache-cassandra-api-of-azure-cosmos-db"></a>Is it possible to create multiple tables with Apache Cassandra API of Azure Cosmos DB?
...
### <a name="is-it-possible-to-create-multiple-tables-in-succession"></a>Is it possible to create multiple tables in succession?
...
### <a name="what-is-maximum-number-of-tables-which-can-be-created"></a>What is maximum number of tables which can be created?
...
### <a name="what-is-the-maximum--of-keyspace-which-we-can-create"></a>What is the maximum # of keyspace which we can create? 
...
### <a name="what-happens-with-respect-to-various-settings-for-table-metadata-like-bloom-filter-caching-read-repair-change-gcgrace-compression-memtableflushperiod-etc"></a>What happens with respect to various settings for table metadata like bloom filter, caching, read repair change, gc_grace, compression memtable_flush_period etc?
...
### <a name="can-i-use-sstable-loader-for-data-loading"></a>Can I use sstable loader for data loading?
No, during preview sstable loader is not supported. 

### <a name="can-an-on-premises-cassandra-cluster-be-paired-with-azure-cosmos-dbs-apache-cassandra-api"></a>Can an on-premises cassandra cluster be paired with Azure Cosmos DB's Apache Cassandra API?
...
### <a name="feature-x-of-regular-cassandra-api-is-not-working-as-today-where-can-the-feedback-be-provided"></a>Feature x of regular Cassandra API is not working as today, where can the feedback be provided?
