...
---
# <a name="performance-tips-for-azure-cosmos-db-and-async-java"></a>Performance tips for Azure Cosmos DB and Async Java
...
9. **Use Appropriate Scheduler (Avoid stealing Event loop IO Netty threads)**
    ...
    Based on the type of your work you should use the appropriate existing RxJava Scheduler for your work. Read here [``Schedulers``](http://reactivex.io/RxJava/1.x/javadoc/rx/schedulers/Schedulers.html).
...
11. **OS Open files Resource Limit** Some Linux systems (like Red Hat) have an upper limit on the number of open files and so the total number of connections. Run the following to view the current limits:
...
12. **Use native SSL implementation for netty** Netty can use OpenSSL directly for SSL implementation stack to achieve better performance. In the absence of this configuration netty will fall back to Java's default SSL implementation.
