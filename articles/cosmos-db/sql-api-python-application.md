...
---
# <a name="build-a-python-flask-web-application-using-azure-cosmos-db"></a>Build a Python Flask web application using Azure Cosmos DB
...
## <a name="step-1-create-an-azure-cosmos-db-database-account"></a>Step 1: Create an Azure Cosmos DB database account
...
## <a name="step-5-deploy-the-web-application-to-azure"></a>Step 5: Deploy the web application to Azure
Now that you have the complete application working correctly against Azure Cosmos DB locally, we're going to create a web.config file, update the files on the server to match the local environment, and then view the completed app on Azure. This procedure is specific to Visual Studio 2017. If you are using a different version of Visual Studio, see [Publishing to Azure App Service](/visualstudio/python/publishing-to-azure).
