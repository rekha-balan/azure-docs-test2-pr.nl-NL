...
---
# <a name="set-up-a-cicd-pipeline-with-the-azure-cosmos-db-emulator-build-task-in-visual-studio-team-services"></a>Set up a CI/CD pipeline with the Azure Cosmos DB emulator build task in Visual Studio Team Services
...
## <a name="install-the-emulator-build-task"></a>Install the emulator build task
...
![Create a new build definition](./media/tutorial-setup-ci-cd/CreateNewBuildDef_1.png) Select the desired team project, repository, and branch to enable builds. 
...
![Add the Emulator build task to the build definition](./media/tutorial-setup-ci-cd/addExtension_3.png) In this tutorial, we'll add the task to the beginning of Phase 1 to ensure the emulator is available before our tests execute.
...
## <a name="configure-tests-to-use-the-emulator"></a>Configure tests to use the emulator
...
## <a name="run-the-build"></a>Run the build
