...
---
# <a name="tutorial-single-page-video-search-app"></a>Tutorial: Single-page Video Search app
...
## <a name="app-components"></a>App components
...
## <a name="managing-subscription-key"></a>Managing subscription key
...
```
## <a name="selecting-search-options"></a>Selecting search options
...
## <a name="performing-the-request"></a>Performing the request
...
## <a name="displaying-search-results"></a>Displaying search results
...
```
The search results are returned as the top-level `value` object in the JSON response. We pass them to our function `renderResultsItems()`, which iterates through them and calls a function to render each item into HTML. The resulting HTML is returned to `renderSearchResults()`, where it is inserted into the `results` division in the page.
...
## <a name="rendering-result-items"></a>Rendering result items
