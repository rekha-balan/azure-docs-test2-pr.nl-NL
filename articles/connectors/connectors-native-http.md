...
5. On the designer toolbar, click **Save**. Your logic app is saved and published (activated) at the same time.
...
> [!TIP]
> If you want to use a password that cannot be retrieved from the definition, use a `securestring` parameter and the `@parameters()` 
...
> [!TIP]
> To use a parameter that won't be readable in the definition after saving the logic app, you can use a `securestring` parameter and the `@parameters()` 
> [workflow definition function](http://aka.ms/logicappdocs).
