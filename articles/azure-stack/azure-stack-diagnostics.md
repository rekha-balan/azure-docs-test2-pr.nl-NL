...
### <a name="to-run-get-azurestacklog-on-azure-stack-integrated-systems"></a>To run Get-AzureStackLog on Azure Stack integrated systems 
...
### <a name="to-run-get-azurestacklog-on-an-azure-stack-development-kit-asdk-system"></a>To run Get-AzureStackLog on an Azure Stack Development Kit (ASDK) system
...
### <a name="parameter-considerations-for-both-asdk-and-integrated-systems"></a>Parameter considerations for both ASDK and integrated systems

- If the **FromDate** and **ToDate** parameters are not specified, logs are collected for the past four hours by default.
- Use the **FilterByNode** parameter to filter logs by computer name. For example: ```Get-AzureStackLog -OutputPath <path> -FilterByNode azs-xrp01```
- Use the **FilterByLogType** parameter to filter logs by type. You can choose to filter by File, Share or WindowsEvent. For example: ```Get-AzureStackLog -OutputPath <path> -FilterByLogType File```
