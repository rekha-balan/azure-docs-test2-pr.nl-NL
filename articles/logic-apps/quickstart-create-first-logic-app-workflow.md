...
---
# <a name="quickstart-create-your-first-automated-workflow-with-azure-logic-apps---azure-portal"></a>Quickstart: Create your first automated workflow with Azure Logic Apps - Azure portal
...
Also, if you don't have an Azure subscription, <a href="https://azure.microsoft.com/free/" target="_blank">sign up for a free Azure account</a>.
...
3. After Azure deploys your app, the Logic Apps Designer opens and shows a page with an introduction video and commonly used triggers. Under **Templates**, choose **Blank Logic App**.
...
## <a name="check-rss-feed-with-a-trigger"></a>Check RSS feed with a trigger

1. On the designer, enter "rss" in the search box. Select this trigger: **RSS - When a feed item is published**
...
3. To hide the trigger details for now, click inside the trigger's title bar.
...
## <a name="send-email-with-an-action"></a>Send email with an action
...
3. If asked for credentials, sign in to your email account so that Logic Apps creates a connection to your email account.
...
4. In the **Send an email** action, specify the data that you want the email to include. 
   ...
   For testing purposes, you can use your own email address.
      ...
      When you click inside some edit boxes, this list appears and shows any available parameters from the previous step that you can include as inputs in your workflow. 
   ...
   4. In the **Body** box, enter this text, and select these tokens for the email body. 
